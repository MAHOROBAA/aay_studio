...
### 더보기 메뉴 "수정" 연결 (세계관·스토리만)

배치 B 이후로 라이브러리 목록의 더보기 메뉴 중 "삭제"만 실제 동작하고 나머지(수정/다운로드/게시 설정 변경/
예약 변경)는 전부 `onSelect: () => {}` 상태였다. 이번에 이 중 **세계관·스토리의 "수정"만** 연결했다 —
배치 D에서 이미 만든 수정 폼(`/library/worlds/:worldId/edit`, `/library/stories/:storyId/edit`)으로
`navigate`만 추가하면 되는, 이미 완성된 화면을 연결하는 안전한 작업이었다.

나머지는 사용자 확인 후 이번 배치에 포함하지 않았다:
- **캐릭터 "수정"**: Figma에 캐릭터 "생성" 팝업만 있고 수정 전용 화면/디자인이 없다. `CharacterCreatePopup`을
  수정 모드로 확장해서 재사용하는 방안이 유력하지만, 화면을 새로 설계하는 셈이라 별도 확인 후 진행하기로 했다.
- **영상 "다운로드"**: 연결할 실제 파일이 없다(파일 저장소는 spec 11장 4단계 항목).

### 영상 "게시 설정 변경"/"예약 변경" 추가 구현

위에서 보류했던 "게시 설정 변경"/"예약 변경"을 사용자가 직접 정책과 화면 형태를 지정해줘서 이어서 구현했다.

- **정책 변경(addendum 7.2 수정)**: "게시 완료" 상태는 이미 게시가 끝나서 설정을 바꿀 수 없다는 사용자 판단에
  따라 더보기 메뉴에서 "게시 설정 변경" 항목 자체를 뺐다(현재 다운로드/삭제만 남음). addendum 문서엔 "게시
  완료: 영상 상세, 게시 설정 변경, 삭제"로 되어 있지만, 이번 지시로 "게시 설정 변경"은 "게시 실패" 상태에서만
  노출한다. addendum 원본 파일은 사용자가 직접 관리하는 문서라 임의로 고치지 않고 여기(구현 노트)에만 반영했다
  — addendum이 갱신되면 그쪽을 기준으로 다시 맞춘다.
- **화면**: Figma에 이 화면 디자인이 없어서, 사용자 지시대로 "세계관/스토리 수정 폼"과 똑같은 페이지 형태
  (`LibraryWorldFormPage`와 동일한 셸: `<h1>` 타이틀 + `.row`(라벨+인풋) 반복 + 우측 정렬 액션 버튼 2개)를
  그대로 재사용해서 새 페이지 `LibraryVideoPublishSettingsPage`(`/library/videos/:videoId/publish-settings`)를
  만들었다. 필드는 `CreatePublishPage`의 게시 설정 필드(제목/게시글 내용/시청자층/공개 범위)에 "게시 방식"
  (지금 게시/예약 게시)과, 예약 게시를 선택했을 때만 나타나는 "예약 일시" 입력을 추가했다.
  - 최종 메뉴 구성(사용자가 여러 차례 시행착오 끝에 확정): **게시 완료 → 다운로드/공개 설정 변경/삭제**,
    **게시 실패 → 다운로드/게시 상태 변경/삭제**, **예약 게시 → 다운로드/예약 변경/삭제**. 세 항목 모두 같은
    페이지(`/library/videos/:videoId/publish-settings`)로 연결되고, 목록 메뉴가 `navigate(..., { state: {
    status: video.status } })`로 상태값을 넘겨서 페이지가 상태별로 다르게 렌더링한다.
  - **"게시 완료"(공개 설정 변경) 진입 시에는 "공개 범위"만 수정 가능하고 나머지(제목/게시글 내용/시청자층/
    게시 방식)는 전부 읽기 전용**이다 — 이미 게시가 끝나서 공개 범위 말고는 바꿀 수 없다는 사용자 지시를
    반영했다(`isPublished` 플래그로 분기, `LibraryVideoDetailPage`의 `.metaBox`와 같은 톤의 읽기 전용
    박스 `.readOnlyValue`/`.readOnlyTextarea` 사용). 페이지 제목도 이때만 "공개 설정 변경"으로 바뀐다.
  - **"게시 실패"(게시 상태 변경)/"예약 게시"(예약 변경) 진입 시에는 모든 항목이 그대로 수정 가능하다** —
    `isPublished`가 `status === '게시 완료'`일 때만 참이 되므로 나머지 두 상태는 이미 전부 편집 가능한
    상태다.
  - 제출 버튼 문구는 게시 방식에 따라 "예약 변경 완료"/"다시 게시"로 바뀐다. 세계관/스토리 폼과 마찬가지로
    실제 목록 데이터와 연결돼 있지 않아(항상 같은 mock 값으로 채워짐, 제출해도 목록에 반영 안 됨) 이 폼도
    동일한 한계를 가진다 — 4단계 데이터 저장 연동 이후 제대로 연결된다.
