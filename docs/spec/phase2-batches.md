...
## 배치 G 확인 결과 (완료) — 마이페이지 + 크레딧 UI

- `spec-addendum-credit.md`가 추가된 뒤 Figma에도 실제 마이페이지 크레딧 UI가 반영돼 있었다(`105:234`의 "Credit Summary"
  섹션과 새 화면 `176:1145`(크레딧 내역)/`176:1231`(크레딧 충전)). Figma가 이미 addendum 14장·8~10장 내용을 화면으로
  구체화해뒀으므로, 텍스트 스펙(예: 이용내역 필터 "전체/충전/지급/사용/반환/만료")보다 Figma의 실제 화면(탭 2개:
  충전내역/사용내역)을 그대로 따랐다 — 문서 우선순위 규칙(0장)대로 화면 구조는 Figma가 우선.
  - `/mypage`: 크레딧 요약(잔액+충전 버튼+충전내역·사용내역 링크), 연결 계정(YouTube/TikTok), 계정 정보, 서비스 정보,
    계정 관리(탈퇴) 5개 섹션.
  - `/mypage/credits`: 크레딧 내역(잔액 요약 + 충전내역/사용내역 탭 + 조회 기간 필터 + 테이블).
  - `/mypage/credits/charge`: 크레딧 충전(잔액 요약 + 크레딧충전/주간제작지원 탭 + 상품 목록 + 선택 상품 요약 + 결제하기 + 결제 안내).
- **탭이 하나만 디자인된 경우 인터랙션을 넣지 않았다**: 크레딧 내역의 "사용 내역" 탭, 크레딧 충전의 "주간 제작 지원" 탭은
  Figma에 해당 탭의 실제 콘텐츠가 없다(첫 번째 탭만 디자인됨). 상태를 토글하되 콘텐츠가 바뀌지 않는 것은 사용자에게
  고장난 것처럼 보일 수 있어서, 두 화면 모두 탭 버튼은 정적으로만 두고(첫 탭이 항상 활성 표시) 클릭 인터랙션은 넣지 않았다.
  반대로 크레딧 충전의 **상품 카드 선택(5개 모두 일반/선택 두 상태가 Figma에 있음)은 실제로 동작하게 구현**했다 —
  클릭 시 선택 카드 테두리와 우측 "선택 상품 요약" 패널이 함께 바뀐다.
- **버튼 중 대상 화면/플로우가 없는 것들은 Header의 로그아웃 버튼과 동일하게 처리**했다: 클릭 핸들러 없이 그냥 렌더링만
  한다(연결 계정의 "변경"/"연결", 계정 정보의 "로그아웃", 계정 관리의 "계정 탈퇴", 서비스 정보의 3개 행, 크레딧 충전의
  "결제하기"). "새 캐릭터 만들기"처럼 `disabled`로 죽이지는 않았다 — 이 버튼들은 Figma상 항상 활성 상태로 보이는 게
  맞고, 단지 뒤에 연결할 화면/팝업(계정 탈퇴 위험 컨펌, 결제 Mock 흐름 등)이 아직 없을 뿐이라 "새 캐릭터 만들기"
  케이스와는 성격이 다르다고 판단했다.
- **크레딧 충전 결제 버튼은 Mock 결제 흐름도 연결하지 않았다.** addendum 24장이 "현재 프론트엔드 단계"에 결제 Mock
  흐름·성공/실패 화면을 포함하고 있지만, Figma에는 아직 결제 성공/실패 화면이나 잔액 부족 팝업이 없다(화면이 없는데
  임의로 만들지 않는다는 원칙). 실제 팝업/화면이 Figma에 추가되면 그때 연결한다.
- **네비게이션 구조**: 마이페이지의 "크레딧 충전" 버튼과 "충전내역·사용내역" 링크는 각각 `/mypage/credits/charge`,
  `/mypage/credits`로 이동한다. 크레딧 충전 화면의 "상세 내역" 버튼은 `/mypage/credits`로 이동한다. 세 화면 모두
  Figma에 뒤로가기 버튼이 없어서(라이브러리 상세 페이지와 다름) 임의로 추가하지 않았다 — LNB의 "마이페이지" 메뉴로
  돌아가는 것을 전제로 한 구조로 보인다.
- **아이콘 정리**: 서비스 정보 행의 `>` 아이콘은 기존 정렬 드롭다운 화살표(`▾`, 아래 방향)와 실제로 다른 아이콘
  (오른쪽 방향 `chevron-right`)이라 새로 `ChevronRightIcon`을 만들었다. 동시에, 이번에 세 번째로 중복되려던 기존
  정렬 화살표 코드(`LibraryVideosPage`/`LibraryCharactersPage`에 각각 인라인으로 있던 동일한 SVG)를
  `src/components/common/ChevronDownIcon/ChevronDownIcon.tsx`로 공통화하고 두 페이지도 이걸 쓰도록 정리했다.
...
- **아이콘 공통 컴포넌트**: `MoreIcon`(⋮), `GridViewIcon`/`ListViewIcon`(보기 전환), `ChevronDownIcon`(아래 방향, 정렬·필터
  드롭다운용), `ChevronRightIcon`(오른쪽 방향, 서비스 정보 같은 이동형 행의 화살표용) — 전부 `src/components/common/`에
  있고 Figma 원본 SVG 경로를 그대로 옮겼다. 새로 유사한 화살표/아이콘이 필요하면 먼저 이 목록에서 재사용 가능한지 확인한다.
