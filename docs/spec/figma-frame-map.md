...
- "직접 만들기 - stepN"이 스펙 5.5~5.8의 어느 화면에 해당하는지는 아직 확인 전이다(2단계 배치 C 작업 시 확인 예정). step3~5(`39:160`, `63:1211`, `63:1504`)는 템플릿 플로우의 step3~5와 마찬가지로 `/create/settings`·`/create/generating`·`/create/review`를 공유할 가능성이 높다 — 실제로 열어서 확인 후 이 표를 갱신한다.
- "라이브러리 - 캐릭터"(`133:265`), "라이브러리 - 세계관"(`133:444`)은 스펙 문서의 라우트 표(4.2)에 없는 화면이다. 별도 캐릭터/세계관 관리 화면인지, MVP 범위(3.4 "추후 기능"의 "별도 캐릭터 관리 메뉴")에 해당하는지 확인이 필요하므로 임의로 구현하지 않는다.
