...
export class AiUsageLogService {
  ...
  async record(entry: AiUsageLogEntry): Promise<void> {
    const { error } = await this.insertRow('ai_usage_logs', {
      ...
      input_image_count: entry.inputImageCount ?? null,
      output_image_count: entry.outputImageCount ?? null,
      video_duration_seconds: entry.videoDurationSeconds ?? null,
      output_resolution: entry.outputResolution ?? null,
